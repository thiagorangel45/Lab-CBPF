...
G4VPhysicalVolume *PMDetectorConstruction::Construct()
{
    ...
    // Definição correta da fonte radioativa
    G4double sourceRadius = 10. * cm; // Reduzindo o tamanho para melhor visualização
    ...
    G4VisAttributes *sourceVisAtt = new G4VisAttributes(G4Colour(1.0, 1.0, 0.0)); // Alterado para amarelo
    sourceVisAtt->SetVisibility(true);
    sourceVisAtt->SetForceSolid(true);
    sourceVisAtt->SetForceAuxEdgeVisible(true); // Garante que as bordas da esfera sejam visíveis
